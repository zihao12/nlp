...
class WAC_ATTR2(nn.Module):

	def __init__(self, embedding_dim, vocab_size):
		super(WAC_ATTR2, self).__init__()
		self.word_embeddings = nn.Embedding(vocab_size, embedding_dim, sparse = True)
		self.word_embeddings2 = nn.Embedding(vocab_size, embedding_dim, sparse = True)
	...
	def forward(self, X, lens):
		...
		## compute attention
		sim = torch.mul(self.word_embeddings2(X), embeds).sum(dim = 2) ## [bs, sentlen]
